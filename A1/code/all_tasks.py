...
plt.axhline(E0_arr[-1], linestyle="--", color="k", alpha=0.5, label=f"$E_0 = {E0_arr[-1]:.5f} \\: / \\:$atomic units")
...
plt.plot(E_arr)
plt.axvline(len(r_max_arr)-1, linestyle=":", color="k", alpha=0.5, label="$r_{max}$ is converged")
plt.axhline(E_arr[-1], linestyle="--", color="k", alpha=0.5, label=f"$E = {E_arr[-1]:.5f} \\: / \\:$atomic units")
plt.xlabel("run index")
plt.ylabel(r"$E \: / \:$atomic units")
plt.legend()
plt.show()
...
plt.plot(r_max_arr)
plt.xlabel("run index")
plt.ylabel(r"$r_{max} \: / \:$atomic units")
plt.show()

plt.figure(figsize=(5,4))
plt.plot(np.arange(len(r_max_arr)-1,len(E_arr)), dr_arr)
plt.xlabel("run index")
plt.ylabel(r"$\Delta r \: / \:$atomic units")
plt.show()
